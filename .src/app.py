...
def add_to_cart(name, price, unit):
...
with col_left:
    ...
    if selected_item_search != "":
        ...
        st.info(f"分類: {item_info['category']} | 單價: ${item_info['price']} / {item_info['unit']}")
        st.button("➕ 從搜尋加入", key="btn_search_add", type="primary", use_container_width=True, 
                  on_click=add_to_cart, args=(selected_item_search, item_info['price'], item_info['unit']))
